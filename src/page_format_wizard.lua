...
local function format_wizard()
    ...
    local function page_size_update(controls, page_settings, ctrls_collection)
        ...
        controls.left_page_top_edit:SetMeasurement(page_settings.first_page_top_margin, page_settings.page_units)
        ...
        if page_settings.reflect_bool == 1 then
            ...
            for k,v in ipairs(ctrls_collection.right_page_ctrls_2) do
                v:SetEnable(false)
            end
        else
            for k,v in ipairs(ctrls_collection.right_page_ctrls_2) do
        ...
        if page_settings.staff_spacing == 0 then
            for k, v in pairs(ctrls_collection.staff_spacing_ctrls) do
                v:SetVisible(false)
            end
    ...
    local function format_pages_and_save()
        ...
        local function copy_format_prefs(page_settings, page_format_prefs)
            ...
            page_format_prefs:SetLeftPageTopMargin(page_settings.first_page_top_margin)
    ...
    local function section_create(controls, page_settings, ctrls_collection, section_n)
        ...
        for i,k in ipairs(page_sizes) do
