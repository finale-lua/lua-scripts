...
--
    ...
    for i, j in pairs(staves) do
      ...
      edit_abbname[i] = add_ctrl(dialog, "edit", abbnames[i], col[2], row[i + 1], row_h, col_w * .75, 0, 0)
      copy_button[i] = add_ctrl(dialog, "button", "→", col[2] - col_gap + 2, row[i + 1]-2, row_h-4, 16, 0, 0)
      row_count = row_count + 1
    end
    --
    local form_select = add_ctrl(dialog, "popup", "", col[1], row[row_count + 1] + row_h/2, row_h, col_w - col_gap, 0, 0)
    local forms = {"Inst. in T","T Inst."}
