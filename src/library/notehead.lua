...
local config = {
    ...
    wedge_whole = 231,
    wedge_breve = 231,
    wedge_hollow_resize = 100,
    wedge_whole_resize = 100,
    wedge_breve_resize = 100,
    wedge_whole_offset = -14,
    wedge_breve_offset = -14,
    hidden = 202,
    default_notehead = 207,
}

-- allowable names for notehead families
...
if library.is_font_smufl_font() then
    ...
    config.wedge_filled = 0xe1c5
    config.wedge_hollow = 0xe1c8
    config.wedge_whole = 0xe1c4
    config.wedge_breve = 0xe1ca
    config.wedge_hollow_resize = 120
    config.wedge_whole_resize = 120
    config.wedge_breve_resize = 120
    config.wedge_whole_offset = 0
    config.wedge_breve_offset = 0
...
function notehead.change_shape(note, shape)
    ...
    if shape == "default" then
        ...
        -- shape == "number" requires no further modification
        ...
        elseif shape == "wedge" then
            notehead_char = config.wedge_filled
            if duration >= finale.BREVE then
                notehead_char = config.wedge_breve
                resize = config.wedge_breve_resize
                offset = config.wedge_breve_offset
            elseif duration >= finale.WHOLE_NOTE then
                notehead_char = config.wedge_whole
                resize = config.wedge_whole_resize
                offset = config.wedge_whole_offset
            elseif duration >= finale.HALF_NOTE then
                notehead_char = config.wedge_hollow
                resize = config.wedge_hollow_resize
        end
        --  --------
        elseif shape == "x" then
