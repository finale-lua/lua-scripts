...
local function clean_beams(entry)
    ...
    for _, v in ipairs{false, true, nil} do
        beam:SetNoteEntry(entry)
        if v ~= nil then beam:UseUpStemData(v) end
...
local function change_beam_height(entry, offset)
    ...
    if not entry.FreezeStem and config.direction == "Up" then -- not at start of beam group
        use_upstem = true
    end
    local beam = finale.FCBeamMod(false)
    beam:SetNoteEntry(entry)
...
local function get_staff_metrics(measure, staff_number)
    ...
    local staff_top = cell_metrics.ReferenceLinePos
    staff_top = staff_top - 48 -- (?* staff_scale) adjust for middle staff line
    cell_metrics:FreeMetrics()
    return staff_top, staff_scale
end
...
local function cross_staff(dialog)
    ...
    local count, active_beam = 0, nil   -- track the start entry of each beam
...
local function run_the_dialog()
    ...
    for i, v in ipairs(offsets) do -- OFFSET MEASUREMENTS
        ...
        if v[3] then
            answer["off" .. i] = cstat(v[4], y, v[3], x[1]) -- describe the entry values
        end
    end
    dy(20)
    ...
    dialog:RegisterHandleOkButtonPressed(function(self)
        ...
        user_error = submission_error(self) or (not cross_staff(self)) -- error if eligible failed
    end)
