...
function plugindef()
    ...
    finaleplugin.RTFNotes = [[
        ...
        {\pard \sl264 \slmult1 \ql \f0 \sa0 \li360 \fi-360 \bullet \tx360\tab Remove \u8220"real\u8221" whole rests from fermata measures. This issue arises when the fermata was attached to a \u8220"real\u8221" whole rest. The fermata is retained in the MusicXML, but the whole rest is removed. This makes for a better import, especially into MuseScore 4.\par}
        {\pard \sl264 \slmult1 \ql \f0 \sa0 \li360 \fi-360 \bullet \tx360\tab When a parallel {\f1 .musx} or {\f1 .mus} file is found, changes all rests in the MusicXML to floating if they were floating rests in the original Finale file. (You can suppress this by processing your xml files from a different folder than the Finale file.)\sa180\par}
        ...
        {\pard \sl264 \slmult1 \ql \f0 \sa180 \li0 \fi0 When tracking a Finale document, certain situations will cause the script to log warnings and stop processing a measure/staff cell for floating rests. The most common are\par}
        {\pard \sl264 \slmult1 \ql \f0 \sa0 \li360 \fi-360 \bullet \tx360\tab cross staff notes\par}
        {\pard \sl264 \slmult1 \ql \f0 \sa0 \li360 \fi-360 \bullet \tx360\tab beams over barlines made with the Beam Over Barline plugin\sa180\par}
        {\pard \sl264 \slmult1 \ql \f0 \sa180 \li0 \fi0 The script is as conservative as possible, so generally you can ignore these warnings. Your best bet is to import the resulting massaged xml file and see if you prefer it to the original. The log file is named {\f1 FinaleMassageMusicXMLLog.txt} and is to be found in the base folder from which you started processing.\par}
        }
    ]]
...
function directions_of_type(node, node_name)
    ...
    local function find_next_octave_shift()
        while child do
            ...
            child = child:NextSiblingElement("direction")
        end
    end
...
function process_one_file(input_file)
    ...
    local function abort_if(condition, msg)
        if condition then
            ...
            os.remove(text.convert_encoding(output_file, text.get_utf8_codepage(), text.get_default_codepage()))
            close_document()
            return true
        end
        return false
