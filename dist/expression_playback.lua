...
function plugindef()
    ...
    finaleplugin.RTFNotes = [[
        ...
        {\pard \sl264 \slmult1 \ql \f0 \sa180 \li0 \fi0 Change the assigned playback layer and position for all expressions in the current selection. Layers {\b 1}-{\b 4} are the {\i standard} playback layers. Layer numbers {\b 0}, {\b 5} and {\b 6} are interpreted respectively as {\b Current}, {\b Chord} and {\b Expression} Layers for playback.\par}
        {\pard \sl264 \slmult1 \ql \f0 \sa180 \li0 \fi0 Hold down {\i Shift} when starting the script to repeat the same action as last time without a confirmation dialog.\par}
        }
    ]]
    finaleplugin.HashURL = "https://raw.githubusercontent.com/finale-lua/lua-scripts/master/hash/expression_playback.hash"
...
local function user_dialog()
    ...
    dialog:RegisterHandleOkButtonPressed(function()
        ...
        c.start_at = dialog:GetControl("start_at"):GetSelectedItem()
    end)
...
local function playback_layer()
    ...
    if mod_key or user_dialog() then
        local start_option = start_options[2][c.start_at + 1]
