...
function plugindef()
    ...
    finaleplugin.RTFNotes = [[
        ...
        {\pard \sl264 \slmult1 \ql \f0 \sa180 \li0 \fi0 This script presents an alphabetical list of 24 individual types of data to delete, each line beginning with a configurable \u8220"hotkey\u8221". Call the script, type the hotkey and hit [Enter] or [Return]. Half of the datatypes can be filtered by layer.\par}
        ...
        {\pard \sl264 \slmult1 \ql \f0 \sa180 \li0 \fi0 To delete the same data as last time without a confirmation dialog hold down [shift] when starting the script. The layer number is \u8220"clamped\u8221" to a single character so to change layer just type a new number - \u8216'delete\u8217' key not needed.\par}
        }
    ]]
...
function shape_check_layer(shape, layer_num)
    if layer_num == 0 then return true end
...
local function delete_selected(delete_type)
    ...
    if delete_type == "user_selected" then
    ...
    elseif delete_type:find("express") then
    ...
    elseif delete_type == "chords" then
    ...
    elseif delete_type == "staff_styles" then
        for staff_number in eachstaff(rgn) do
            ...
            for style in eachbackwards(style_assign) do
                ...
                if (ss.L <= rr.R and ss.R >= rr.L) then
                    if (ss.L >= rr.L) and (ss.R <= rr.R) then
                        ...
                        if ss.L >= rr.L then
                            ...
                            if ss.R > rr.R then
                                ...
                                style_copy:SetStyleID(style.StyleID)
                                    :SetStartMeasure(rr.R + 1)
                                    :SetEndMeasure(ss.R)
