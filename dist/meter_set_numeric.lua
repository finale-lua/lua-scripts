...
function user_chooses_meter(meter, rgn)
    ...
    clear_button:AddHandleCommand(function()
            box[1]:SetText("4")
            box[4]:SetInteger(4)
            for _, i in ipairs({2, 3, 7, 8, 9}) do
                box[i]:SetText("0")
...
function encode_current_meter(time_sig, sub_meter)
    ...
    if time_sig.CompositeTop then
        ...
        if group_count > 3 then group_count = 3 end
    ...
    if time_sig.CompositeBottom then
        ...
        for i = 1, group_count do
            ...
            sub_meter.bottom[i] = math.floor(4096 / beat_duration)
        end
    else
    ...
    for jump = 0, 6, 6 do
        ...
        for count = 1, 3 do
            ...
            if string.find(value, "^%d") then
                ...
                data.top[count] = { tonumber(choices[c_count]) }
            end

            local bottom = choices[c_count + 3]
            if bottom <= 64 and bottom > 0 then
