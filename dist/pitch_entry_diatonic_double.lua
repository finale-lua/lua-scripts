    ...
    local layer_num = dialog:CreateEdit(x_off + 20, y - offset):SetWidth(20):SetText(save_layer)
        :AddHandleCommand(function(self)
            ...
            if val:find("[^0-" .. max .. "]") then
                if val:find("[+=a]") then list_increment(-1)
                elseif val:find("[-_z]") then list_increment(1)
                ...
                elseif val:find("[e%[]") then octave_increment(-1)
                elseif val:find("[d%]]") then octave_increment(1)
