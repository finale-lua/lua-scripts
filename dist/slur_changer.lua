...
function plugindef()
    ...
    finaleplugin.RTFNotes = [[
        ...
        {\pard \sl264 \slmult1 \ql \f0 \sa180 \li0 \fi0 Change the characteristics of every slur in the current selection. Type the programmable {\i hotkey} and click {\b Apply} [{\i Return}/{\i Enter}]. If slurs are {\b Note-Attached} you can isolate them by {\b Layer Number}. Use this script to change a whole set of slurs at once instead of individually by right-clicking with the {\i SmartShape} tool.\par}
        {\pard \sl264 \slmult1 \ql \f0 \sa180 \li0 \fi0 To repeat the last action without a confirmation dialog hold down {\i Shift} when opening the script.\par}
        }
    ]]
    finaleplugin.HashURL = "https://raw.githubusercontent.com/finale-lua/lua-scripts/master/hash/slur_changer.hash"
...
for _, v in ipairs(dialog_options) do config[v[1]] = v[2] end
...
local function slur_match_layer(slur)
    if config.layer == 0 then return true end
    ...
    return (entry.LayerNumber == config.layer)
end
...
local function run_the_dialog()
    ...
    local key_list = dialog:CreateListBox(0, 22):SetWidth(box_wide):SetHeight(box_high)
        ...
        local function change_keys()
            ...
            else configuration.get_user_settings(script_name, config)
            end
