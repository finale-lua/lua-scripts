...
function plugindef()
    ...
    finaleplugin.RTFNotes = [[
        ...
        {\pard \sl264 \slmult1 \ql \f0 \sa180 \li0 \fi0 Make dynamic marks in the selection louder or softer by stages. This functionality is buried within the {\b JW Change} plugin but is useful enough to make it accessible more easily. This script works similarly but allows jumping up to 9 {\i levels} at once. Dynamics range from {\b pppppp} to {\b ffffff}, though scores using older (non-{\b SMuFL}) fonts are restricted to the range {\b pppp}-{\b ffff}.\par}
        {\pard \sl264 \slmult1 \ql \f0 \sa180 \li0 \fi0 To repeat the previous level shift without a confirmation dialog hold down [{\i Shift}] when starting the script.\par}
        }
    ]]
    finaleplugin.HashURL = "https://raw.githubusercontent.com/finale-lua/lua-scripts/master/hash/dynamic_levels.hash"
...
local dyn_char = library.is_font_smufl_font() and
    {
        0xe527, 0xe528, 0xe529, 0xe52a, 0xe52b, 0xe520, 0xe52c,
        0xe52d, 0xe522, 0xe52f, 0xe530, 0xe531, 0xe532, 0xe533,
    } or
    {
         175, 184, 185, 112,  80,
          70, 102, 196, 236, 235
...
local function change_dynamics(dialog)
    ...
    local found = { show = {}, hide = {} }
    local match_count = { show = 0, hide = 0 }
    local shift = config.levels
    if config.direction == 1 then shift = -shift end
    local dyn_len = library.is_font_smufl_font() and 3 or 2

        local function match_dynamics(hidden)
            ...
            for exp_def in each(exp_defs) do
                if exp_def.CategoryID == 1 and hidden == is_hidden_exp(exp_def) then
                    local str = exp_def:CreateTextString()
                    str:TrimEnigmaTags()
                    if str.LuaString:len() <= dyn_len then
                        for i, v in ipairs(dyn_char) do
                            if not found[mode][i] and str.LuaString == utf8.char(v) then
                                found[mode][i] = exp_def.ItemNo
                                match_count[mode] = match_count[mode] + 1
                            end
                        end
                    end
                end
                if match_count[mode] >= #dyn_char then break end
            end
        end
    match_dynamics(true)
            ...
            if exp_def then
                ...
                if str.LuaString:len() <= dyn_len then
                    for i, v in ipairs(dyn_char) do
                        ...
                        if str.LuaString == utf8.char(v) then
                            if found[mode][target] then
                                e:SetID(found[mode][target]):Save()
                            else
                                if not config.create_new then
                                    config.create_new = create_dynamics_alert(dialog)
                                end
                                if config.create_new then
                                    if dialog then
...
local function run_the_dialog()
    ...
    local softer = ctl.direction:GetItemAt(1)
    softer:SetTop(y + 24)
