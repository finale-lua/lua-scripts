...
function break_secondary_beams()
    ...
    local beamed_length = finale.NOTE_8TH
    for entry in eachentrysaved(finenv.Region()) do
        if measure_number ~= entry.Measure or staff_number ~= entry.Staff then
            ...
            if (beamed_length % 3 == 0) then
                beamed_length = beamed_length / 3
            else
                beamed_length = beamed_length / 2
            end
        end
