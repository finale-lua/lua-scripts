...
function plugindef()
    ...
    finaleplugin.RTFNotes = [[
        ...
        {\pard \sl264 \slmult1 \ql \f0 \sa180 \li0 \fi0 This script is loaded with a set of \u8220"demo\u8221" palettes containing many of the Lua scripts available at {\field{\*\fldinst{HYPERLINK "https://FinaleLua.com"}}{\fldrslt{\ul
        FinaleLua.com
        }}}
        ...
        {\pard \sl264 \slmult1 \ql \f0 \sa180 \li0 \fi0 You can also add Finale menus to your palettes. Not every menu item is available, including Plug-ins that are {\b not} added by {\i RGP Lua}, so when using {\i Add Menu Item} try it out before saving to a palette.\par}
        }
    ]]
    ...
    for cnt = 1, menu.get_item_count(top_menu) do
        ...
        if i_type == menu.ITEMTYPE_SUBMENU or i_type == menu.ITEMTYPE_COMMAND then
            ...
            if not string.find(name, "Plug.ins") then
                ...
                {   text = name,
                    sub = menu.get_item_submenu(top_menu, cnt - 1),
                    id = menu.get_item_command_id(top_menu, cnt - 1),
                }
                index = index + 1
            end
        end
...
Note that many Finale menus do nothing unless part of the score is already selected.
    ...
    inputs[5]:AddHandleCommand(function()
        finenv.UI():ExecuteOSMenuCommand(chosen_now.id)
    end)
    open_submenu:AddHandleCommand(function() one_level_down() end)
    up_to_parent:AddHandleCommand(function()
        if level > 1 then
...
local function configure_palette(palette_number, index_num, parent_window)
    ...
    remove_item:AddHandleCommand(function()
    ...
    reassign_keys:AddHandleCommand(function()
...
local function main()
    ...
    while not finished do
        ok, palette_number = choose_palette(0)
        if ok then
            finished, item_number = choose_palette(palette_number)
            if finished then
