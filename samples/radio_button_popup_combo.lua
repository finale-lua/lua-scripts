function plugindef()
    ...
    finaleplugin.Notes = [[
        ...
        associated popup menu. It uses the auto-layout features of v0.72 to horzontally align the controls.
...
local radio_top = 20
local popup_spacing = utils.win_mac(21, 23)
...
for index, value in ipairs(my_list) do
    local next_vpos = radio_top + popup_spacing*(index - 1) -- popup buttons are spaced out starting with radio_top
    local rad = radio:GetItemAt(index - 1)
    rad:SetText(finale.FCString(value))
    rad:SetTop(next_vpos + utils.win_mac(2,1)) -- popups are offet 1(mac) or 2(win) from the top of the radio buttons
    rad:DoAutoResizeWidth(0) -- radio buttons are set to have a width that exactly matches their text (the 0 parameter does this)
    local popup = dialog:CreatePopup(0, next_vpos)
    popups[index] = popup
    for i = 1, 3 do
